          ...
          <div className="flex flex-wrap gap-2">
            {keywords.map((keyword, index) => (
              <Badge 
                key={keyword.word}
                variant="secondary"
                className="px-3 py-1.5 text-xs animate-fade-in"
                style={{ 
                  animationDelay: `${index * 30}ms`,
                  fontSize: `${Math.max(10, Math.min(14, 8 + keyword.count / 30))}px`
                }}
              >
                {keyword.word}
      ...
      <div className="glass-card p-5">
        ...
        <div className="grid sm:grid-cols-3 gap-4">
          <div className="text-center p-4 bg-muted/50 rounded-lg">
            <p className="text-2xl font-bold text-primary">04/02</p>
            <p className="text-sm text-muted-foreground">Maior volume</p>
            <p className="text-xs text-muted-foreground/70 mt-1">187 menções (+45%)</p>
          </div>
          <div className="text-center p-4 bg-muted/50 rounded-lg">
            <p className="text-2xl font-bold text-success">06/02</p>
            <p className="text-sm text-muted-foreground">Mais positivo</p>
            <p className="text-xs text-muted-foreground/70 mt-1">78% sentimento positivo</p>
          </div>
          <div className="text-center p-4 bg-muted/50 rounded-lg">
            <p className="text-2xl font-bold text-destructive">05/02</p>
            <p className="text-sm text-muted-foreground">Alerta crítico</p>
